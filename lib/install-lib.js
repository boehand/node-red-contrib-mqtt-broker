...
async function ensureInstalled(opts) {
    ...
    const tryLocal = async () => {
        switch (process.platform) {
            case 'linux':  return installLinuxLocal(logger, run);
            case 'darwin': return installMacLocal(logger);
            case 'win32':  return installWindowsLocal(logger, run);
            default:
    ...
    const tryGlobal = async () => {
        switch (process.platform) {
            case 'linux':  return installLinuxGlobal(logger, run);
            case 'darwin': return installMacGlobal(logger, run);
            case 'win32':  return installWindowsGlobal(logger, run);
            default:
    ...
    try {
        if (scope === 'local') {
            ...
            // auto: prefer a private local install (no sudo, no system
            // pollution); only fall back to a system-wide install if the
            // local path isn't usable on this platform.
            ok = await tryLocal();
            if (!ok) {
                logger.log('Local install did not succeed; falling back to global scope.');
                ok = await tryGlobal();
            }
        }
    } catch (err) {
    ...
    const after = scope === 'global' ? findGlobalBinary()
        : scope === 'local' ? findLocalBinary()
        : findBinary();
